 ...
 * banner
 */
const debug = require('debug')('controller:app.banner');
...
exports.createOrUpdate = async(ctx, next) => {
  ...
  query = {
    ...
    type: ctx.checkBody('type').trim().notEmpty(ErrMsg.type).value
  };
  ...
  query.link = query.url;
  query.type === 'native' && (query.url = '');
  if (ctx.errors) {
  ...
  return ctx.redirect('/app/banner');
};
